...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) {
	...
	c.logger.Info("Controller.Run: initialising")

	// run the informer to start listing and watching resource
	go c.informer.Run(stopCh)

	// do the intial sync (one time) to populate resources
...
// necessary handler action based off of it if the item was
// created or deleted
func (c *Controller) processNextItem() bool {
	...
	// we expect strings to come off the queue.
	keyRaw := key.(string)
	...
	// number of times (5 here) before we forget the queue key and
	// throw an error
	...
	// if the item doesn't exists then it was deleted and we need to fire off the handler's
	...
	// a code path of successful queue by processing
	if !exists {
